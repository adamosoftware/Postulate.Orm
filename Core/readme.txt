...
1. Install the Model Merge tool:
   https://adamosoftware.blob.core.windows.net/install/PostulateModelMerge.exe
   ...
   Currently, Model Merge works only with SQL Server. A MySQL version is coming.

   Source for the Model Merge app is here: https://github.com/adamosoftware/Postulate.Orm/tree/master/MergeUI
   ...
   - You can create an External Tool in Visual Studio that launches it. Use this macro in the Arguments:
     ...
     The first time you run it, Model Merge will prompt you for the location of your Models .dll within your solution.
   ...
   - You can create a post-build event on your Models project that calls MergeUI.exe (full path will be in the desktop shortcut) followed by the 
     path to your Models .dll. This will cause Model Merge to offer to sync with the database after every successful build.
...
2. Install either of the platform-specific packages:
   ...
   Both packages have full CRUD support, but at this time only the SqlServer version works with Model Merge.
